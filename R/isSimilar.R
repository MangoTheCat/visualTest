...
#' 
#' \tabular{ll}{
#' Package:  \tab  visualTest \cr
#' Type:  \tab  Package \cr
#' Version:  \tab  0.1 \cr
#' Date:  \tab  April 2014 \cr
#' Lazyload:  \tab  yes \cr
#' }
#' @author ccampbell \email{ccampbell@@mango-solutions.com}
...
isSimilar <- function(file, fingerprint, threshold = 1e-3, exact = FALSE, ...) {
    ...
    } else { stop("fingerprint should be a numeric vector or a single character") }
    
    test <- getFingerprint(file = file, ...)
    
    result <- compareWithFingerprint(test = test, fingerprint = fingerprint, threshold = threshold, exact = exact)
...
compareWithFingerprint <- function(test, fingerprint, threshold = 1e-3, exact = FALSE) {
    ...
    if (exact) {
        ...
        if (all(abs(fivenum(test) - fivenum(fingerprint)) <= abs(threshold))) { result <- TRUE }
    }
