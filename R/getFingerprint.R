...
#' @title Get Image Fingerprint 
#' @param file single character naming PNG or JPG file from which to get fingerprint
#' @param \dots additional arguments
#' @export
#' @examples
#'     getFingerprint(file.path(system.file(package = "visualTest"), "compare", "stest-00.jpg"))
...
getFingerprint <- function(file, ...) {
    ...
    characteristic <- diff(which(zeros))
    
    return(characteristic)
}


#' @title convert RGB array to value
